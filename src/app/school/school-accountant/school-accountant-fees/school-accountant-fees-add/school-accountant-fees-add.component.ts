...
@Component({
  selector: 'app-school-accountant-fees-add',
  templateUrl: './school-accountant-fees-add.component.html',
  styleUrls: ['./school-accountant-fees-add.component.scss']
})
export class SchoolAccountantFeesAddComponent implements OnInit {
  ...
  getStudents() {
    if (this.schoolData._id && this.selectedClass && this.selectedSession) {
      this.student
        .getStudentBySchoolAndClassAndSession(this.schoolData._id, this.selectedClass, this.selectedSession)
        .map(x => x.json())
        .subscribe(res => {
