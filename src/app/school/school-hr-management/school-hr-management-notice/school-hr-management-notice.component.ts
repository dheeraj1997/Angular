...
@Component({
  selector: 'app-school-hr-management-notice',
  templateUrl: './school-hr-management-notice.component.html',
  styleUrls: ['./school-hr-management-notice.component.scss']
})
export class SchoolHrManagementNoticeComponent implements OnInit {
        ...
        if (sres && sres.data && sres.data._id) {
          this.staffData = sres.data;
          {
            ...
            if (sres.data.schoolId) {
              this.notice.getNoticeBySchoolId(sres.data.schoolId, 'hrs')
                .map(z => z.json())
                .subscribe(nres => {
