...
@Component({
  selector: 'app-school-admin-student-view',
  templateUrl: './school-admin-student-view.component.html',
  styleUrls: ['./school-admin-student-view.component.scss']
})
export class SchoolAdminStudentViewComponent implements OnInit {
  ...
  generateAdmissionForm(studentId) {
    this.isLoading = true;
    this.student.getAdmissionForm(studentId)
      .map(x => x.json())
      .subscribe(res => {
        this.isLoading = false;
        window.open('https://inforida.in/student/admission/' + res.filename);
      })
  }

  getIdCards() {
    this.isLoading = true;
    this.student.getIdCards(this.schoolData._id, this.selectedSession, this.selectedClass)
      .map(x => x.json())
      .subscribe(res => {
        this.isLoading = false;
        window.open('https://inforida.in/student/identity/' + res.filename);
      })
  }
}
