...
@Component({
	selector: 'app-school-admin-reports-fee-ledger',
	templateUrl: './school-admin-reports-fee-ledger.component.html',
	styleUrls: ['./school-admin-reports-fee-ledger.component.scss']
})
export class SchoolAdminReportsFeeLedgerComponent implements OnInit {
	schoolData = {
		_id: '',
		name: '',
		currentSession: '',
		...
		if (this.schoolData._id && this.selectedClass && this.selectedSession) {
			...
			.subscribe(res => {
				...
				if (res.success && res.data && res.data.length) {
					...
					this.initLedgerData = JSON.stringify(res.data);
				} else {
	...
	generateCsv() {
		const csvData = JSON.parse(this.initLedgerData).reduce((a, p) => {
			const temp = {
				RollNo: p.rollNo,
				Name: p.name
			};
