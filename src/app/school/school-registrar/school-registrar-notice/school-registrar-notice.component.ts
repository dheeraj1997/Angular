...
@Component({
	selector: 'app-school-registrar-notice',
	templateUrl: './school-registrar-notice.component.html',
	styleUrls: ['./school-registrar-notice.component.scss']
})
export class SchoolRegistrarNoticeComponent implements OnInit {
			...
			if (sres && sres.data && sres.data._id) {
				this.staffData = sres.data;
				{
					...
					if (sres.data.schoolId) {
						this.notice.getNoticeBySchoolId(sres.data.schoolId, 'registrars')
						.map(z => z.json())
						.subscribe(nres => {
