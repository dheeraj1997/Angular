...
@Component({
	selector: 'app-school-teacher-homework-edit',
	templateUrl: './school-teacher-homework-edit.component.html',
	styleUrls: ['./school-teacher-homework-edit.component.scss']
})
export class SchoolTeacherHomeworkEditComponent implements OnInit {
	...
	ngOnInit() {
		this.route.params.subscribe(params => {
			...
			this.homework.getHomeworkById(homeworkId)
			.map(x => x.json())
			.subscribe(res => {
				console.log('res', res);
				if (res.success) {
	...
	editHomework(){
		...
		console.log('Data' , this.homeworkData);
		this.homework.editHomework(this.homeworkData)
		.map(x=>x.json())
		.subscribe(res4=>{
