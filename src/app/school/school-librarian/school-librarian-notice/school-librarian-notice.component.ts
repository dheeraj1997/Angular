...
@Component({
  selector: 'app-school-librarian-notice',
  templateUrl: './school-librarian-notice.component.html',
  styleUrls: ['./school-librarian-notice.component.scss']
})
export class SchoolLibrarianNoticeComponent implements OnInit {
        ...
        if (sres && sres.data && sres.data._id) {
          this.staffData = sres.data;
          {
            ...
            if (sres.data.schoolId) {
              this.notice.getNoticeBySchoolId(sres.data.schoolId, 'librarians')
                .map(z => z.json())
                .subscribe(nres => {
